                    ...
                    <form onSubmit={registerUserIn}>
                        <div className="row mb-4">
                            <div className="form-outline col-6">
                                ...
                                <input type="text" value={last_name} onChange={(e) => {setLastName(e.target.value);lastNameVerif(e.target.value)}} className={`form-control form-control-lg ${last_name_error ? 'is-invalid' : form_submited ? 'is-valid' : ''}`} placeholder="Nom"/>
                                <div className="invalid-feedback">{last_name_error}</div>
                            </div>
                            <div className="form-outline col-6">
                                <label className="form-label">Prénom</label>
                                <input type="text" value={first_name} onChange={(e) => {setFirstName(e.target.value);firstNameVerif(e.target.value)}} className={`form-control form-control-lg ${first_name_error ? 'is-invalid' : form_submited ? 'is-valid' : ''}`} placeholder="Prénom"/>
                                <div className="invalid-feedback">{first_name_error}</div>
                            </div>
                        </div>
                        <div className="form-outline mb-4">
                            <label className="form-label">Adresse mail</label>
                            <input type="email" value={email} onChange={(e) => {setEmail(e.target.value);emailVerif(e.target.value)}} className={`form-control form-control-lg ${email_error ? 'is-invalid' : form_submited ? 'is-valid' : ''}`} placeholder="example@email.com"/>
                            <div className="invalid-feedback">{email_error}</div>
                        </div>

                        <div className="form-outline mb-3">
                            <label className="form-label">Mot de passe</label>
                            <input type="password" value={password} onChange={(e) => {setPassword(e.target.value);passwordVerif(e.target.value)}} className={`form-control form-control-lg ${password_error ? 'is-invalid' : form_submited ? 'is-valid' : ''}`} placeholder="Entrer votre mot de passe."/>
